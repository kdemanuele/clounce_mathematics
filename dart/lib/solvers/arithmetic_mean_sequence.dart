...
class ArithmeticMeanSequence {
  static void solve(num sequenceStartValue, num sequenceEndValue, List<num> variables) {
    ...
    var sequenceNumber = JacobsthalNumber.compute(numberOfVariables + 1);
    var nextSequenceNumber = JacobsthalNumber.compute(numberOfVariables + 2);
    ...
    for (var i = 1; i < numberOfVariables; i++) {
        sequenceNumber = JacobsthalNumber.compute(i + 1);
        nextSequenceNumber = JacobsthalNumber.compute(i + 2);
  ...
  static num solveForVariable(num sequenceStartValue, num sequenceEndValue, int numberOfVariables, int variableToSolve) {
    if (sequenceStartValue == 0 || sequenceEndValue == 0 || numberOfVariables == 0 || variableToSolve == 0 ||
        ...
        var nthJacobsthal = JacobsthalNumber.compute(variableToSolve + 1);
        var preNthJacobsthal = JacobsthalNumber.compute(variableToSolve);
        var sizeVarJacobsthal = JacobsthalNumber.compute(numberOfVariables + 1);
        var nextSizeVarJacobsthal = JacobsthalNumber.compute(numberOfVariables + 2);
